import type { SpindleFrontendContext, SpindleModalHandle, SpindleSelectHandle } from 'lumiverse-spindle-types'
import {
  MAX_WEAVE_LENGTH,
  MAX_DIRECT_MESSAGE_LENGTH,
  DEFAULT_GENERATION_MAX_TOKENS,
  MAX_GENERATION_MAX_TOKENS,
  MAX_IDENTITY_BACKFILL_BATCH,
...
function isSnapshot(value: unknown): value is TimelineSnapshot {
  return isRecord(value)
    ...
    && Array.isArray(value.state.directThreads)
}
...
function gifDisplayUrl(gifUrl: string, highQuality: boolean): string {
...
interface PendingDirectMessage {
  ...
  content: string
  gifQuery: string
  optimisticMessage: TimelineDirectMessage
}
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  const removeStyle = ctx.dom.addStyle(`
    ...
    .xtl-roster { container-type: inline-size; padding: 14px; background: var(--xtl-surface-raised); }
    ...
    @container (max-width: 520px) { .xtl-roster-list { grid-template-columns: 1fr; } .xtl-roster-actor-card .xtl-actor-card-actions { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
    @media (max-width: 680px) { .xtl-dm-message-row { max-width: 93%; } }
    @media (max-width: 520px) { .xtl-app { padding: 0 9px 24px; } .xtl-header { margin-inline: -9px; padding-inline: 13px; } .xtl-subtitle { display: none; } .xtl-post-body, .xtl-post-source, .xtl-post-gif { margin-left: 0 !important; } .xtl-post-gif { width: 100%; } .xtl-post-actions { margin-left: -6px; } .xtl-post--reply { margin-left: 10px; } .xtl-composer-top, .xtl-settings-row { align-items: flex-start; flex-direction: column; } .xtl-select, .xtl-persona-picker { max-width: 100%; width: 100%; } .xtl-actor-card-actions { margin-left: auto; } .xtl-dm-gif-search, .xtl-dm-gif-chip { margin-left: 0; max-width: 100%; } }
  `)
  ...
  const positionDmComposerForKeyboard = () => {
    requestAnimationFrame(() => {
      if (activeView !== 'dms') return
      const thread = root.querySelector<HTMLElement>('.xtl-dm-thread-scroll')
  ...
  const renderHeader = () => {
    ...
    const selectView = (view: 'timeline' | 'dms') => {
      activeView = view
      if (view === 'dms' && activeDirectThreadId) send({ type: 'read_direct_thread', threadId: activeDirectThreadId })
      render()
      if (view === 'dms') requestAnimationFrame(() => root.scrollIntoView({ block: 'start', behavior: 'auto' }))
    }
    ...
    if (unreadDirectMessageCount) messagesView.appendChild(createElement('span', 'xtl-view-nav-badge', unreadDirectMessageCount > 99 ? '99+' : String(unreadDirectMessageCount)))
    ...
    if (activeView === 'timeline') header.appendChild(jumpToManagement)
  ...
  const renderFollowModal = () => {
    ...
    content.appendChild(createElement(
      'p',
      'xtl-follow-modal-copy',
      followedActorKeys.size
        ? `${followedActorKeys.size} actor${followedActorKeys.size === 1 ? '' : 's'} followed. Follow more to add them to the randomized timeline rotation.`
        : 'Follow actors to add them to the randomized timeline rotation.',
    ))
    ...
    const renderResults = () => {
      ...
      const matchingActors = state.replyActors
        .filter((actor) => actorMatchesSearch(actor, followModalSearch))
        .map((actor) => ({ actor, rank: actorSearchRank(actor, followModalSearch) }))
      ...
      if (!visibleActors.length) {
        const message = state.replyActors.length
          ? 'No actors match that search.'
          : !state.permissions.includes('characters')
            ? 'Character-card access is not enabled. Grant the Characters permission in Extensions, then refresh.'
            : 'No character cards, Lumia DLC items, or active Council members are available yet.'
  ...
  const openFollowModal = () => {
    ...
    if (followModal) {
      renderFollowModal()
      followModal.root.querySelector<HTMLInputElement>('.xtl-actor-search')?.focus()
      return
    }
    ...
    for (const property of ['--xtl-blue', '--xtl-blue-soft', '--xtl-muted']) {
      ...
      if (value) modal.root.style.setProperty(property, value)
    }
    followModal = modal
    modal.onDismiss(() => {
      if (followModal === modal) followModal = null
    })
    renderFollowModal()
    queueMicrotask(() => modal.root.querySelector<HTMLInputElement>('.xtl-actor-search')?.focus())
  }
  ...
  const unreadDirectMessages = (thread: TimelineDirectThread): number => thread.messages
    .filter((message) => message.direction === 'incoming' && message.createdAt > thread.lastReadAt)
    .length

  const directThreads = (state: TimelineSnapshot): TimelineDirectThread[] => [...state.state.directThreads]
    .sort((left, right) => {
  ...
  const activeDirectThread = (): TimelineDirectThread | null => snapshot && activeDirectThreadId
    ? snapshot.state.directThreads.find((thread) => thread.id === activeDirectThreadId) ?? null
    : null

  const updateDmBadge = (state: TimelineSnapshot) => {
    unreadDirectMessageCount = state.state.directThreads.reduce((total, thread) => total + unreadDirectMessages(thread), 0)
  }

  const renderDmError = () => {
    if (!dmError) return null
    const notice = createElement('div', 'xtl-notice xtl-notice--error xtl-dm-thread-notice')
    notice.append(document.createTextNode(dmError))
    const dismiss = button('Dismiss', 'xtl-button xtl-button--quiet')
    dismiss.addEventListener('click', () => {
      dmError = ''
      renderDms()
  ...
  function openDirectConversation(actorKey: string): void {
  ...
  function composeDirectMessage(): void {
    activeView = 'dms'
  ...
  const renderDirectInbox = (state: TimelineSnapshot): HTMLElement => {
    ...
    if (!threads.length) {
      ...
      const start = button('New message', 'xtl-button xtl-button--primary')
      start.disabled = dmBusy || !state.permissions.includes('generation') || state.replyActors.length === 0
      start.addEventListener('click', composeDirectMessage)
    ...
    for (const thread of threads) {
      ...
      nameRow.append(
        createElement('span', 'xtl-dm-list-name', thread.actor.name),
        createElement('span', 'xtl-dm-list-handle', `@${thread.actor.handle}`),
      )
      if (lastMessage) nameRow.appendChild(createElement('span', 'xtl-dm-list-time', relativeTime(lastMessage.createdAt)))
      ...
      if (unread) item.appendChild(createElement('span', 'xtl-dm-unread', unread > 99 ? '99+' : String(unread)))
  ...
  const renderDirectMessage = (
    message: TimelineDirectMessage,
    state: TimelineSnapshot,
    options: { pendingGifQuery?: string } = {},
  ): HTMLElement => {
    ...
    if (!outgoing) row.append(actorAvatar(message.author, 'small'), stack)
    else row.append(stack)
    return row
  }

  const renderDirectComposer = (thread: TimelineDirectThread, state: TimelineSnapshot): HTMLElement => {
    ...
    textarea.addEventListener('input', () => {
      ...
      if (textarea.value !== dmDraft) textarea.value = dmDraft
    ...
    sendButton.addEventListener('click', () => {
      ...
      const author = selectedPersona()
        ?? thread.messages.find((message) => message.direction === 'outgoing')?.author
        ?? {
  ...
  const renderDirectThread = (thread: TimelineDirectThread, state: TimelineSnapshot): HTMLElement => {
    ...
    title.append(
      createElement('div', 'xtl-dm-thread-name', thread.actor.name),
      createElement('div', 'xtl-dm-thread-meta', `@${thread.actor.handle} · private conversation`),
    )
    header.append(back, actorAvatar(thread.actor, 'small'), title)
    main.appendChild(header)
    const errorNotice = renderDmError()
    if (errorNotice) main.appendChild(errorNotice)
    if (dmBusy) main.appendChild(createElement('div', 'xtl-notice xtl-dm-thread-notice', `${dmBusyActorName ?? thread.actor.name} is replying…`))
  ...
  const renderDirectWelcome = (state: TimelineSnapshot): HTMLElement => {
    ...
    copy.append(
      createElement('div', 'xtl-header-mark', '✉'),
      createElement('h3', undefined, selectedDirectActorKey ? 'Start a private conversation' : 'Your messages'),
      createElement('p', undefined, selectedDirectActorKey
        ? 'The actor will send the first DM, then this private thread stays out of the timeline.'
        : 'Choose a conversation from your inbox, or start a new private thread with an actor.'),
    )
    newThread.append(copy)
    if (dmError) {
      const errorNotice = renderDmError()
      if (errorNotice) newThread.appendChild(errorNotice)
    }
    ...
    if (dmBusy) newThread.appendChild(createElement('div', 'xtl-notice', `${dmBusyActorName ?? 'Actor'} is writing a direct message…`))
    welcome.appendChild(newThread)
    main.appendChild(welcome)
    return main
  }

  const renderDmsContent = (): HTMLElement => {
    if (!snapshot) return createElement('div', 'xtl-loading', 'Loading your messages…')
  ...
  const renderRoster = (state: TimelineSnapshot) => {
    ...
    const missingIdentityCount = [...representedActorKeys]
      .filter((key) => availableActorKeys.has(key) && !state.state.actorIdentities[key])
      .length
    ...
    claimMissing.addEventListener('click', () => {
      ...
      const confirmed = tab.root.ownerDocument.defaultView?.confirm(
        `Claim identities for ${batchSize} ${batchSize === 1 ? 'actor' : 'actors'} now? This makes one sidecar generation call per actor.${missingIdentityCount > batchSize ? ` ${missingIdentityCount - batchSize} more will remain for another batch.` : ''}`,
      )
      if (!confirmed) return
      busy = true
      busyActorName = null
    ...
    if (rosterActors.length) {
      for (const actor of rosterActors) {
        ...
        chooseIdentity.title = hasClaimedIdentity
          ? `Have ${actor.name} choose a new display name and @ handle`
          : `Have ${actor.name} choose a display name and @ handle`
    ...
    if (!state.replyActors.length) {
      card.appendChild(createElement(
        ...
        'xtl-roster-access',
        !state.permissions.includes('characters')
          ? 'Character-card access is not enabled for Timeline. Grant the Characters permission in Extensions, then refresh.'
  ...
  const renderSettings = (state: TimelineSnapshot) => {
    ...
    intervalLabels.append(
      ...
      createElement('div', 'xtl-settings-hint', 'The backend chooses one followed actor at random after a delay within this range.'),
    )
    ...
    resetLabels.append(
      ...
      createElement('div', 'xtl-settings-hint', 'Deletes public weaves, reactions, reply threads, and direct messages. Followed actors, claimed identities, their schedule, and saved settings stay in place.'),
    )
    const reset = button('Reset timeline', 'xtl-button xtl-button--danger')
    reset.disabled = busy
    reset.addEventListener('click', () => {
      const confirmed = tab.root.ownerDocument.defaultView?.confirm(
        'Reset this timeline? All public weaves, reactions, reply threads, and direct messages will be deleted. Followed actors, claimed identities, and settings will stay in place.',
      )
      if (!confirmed) return
      draft = ''
      replyToId = null
