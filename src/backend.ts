...
const MAX_ACTOR_DISPLAY_NAME_LENGTH = 50
const MAX_ACTOR_HANDLE_LENGTH = 20
const MAX_SAVED_ACTOR_IDENTITIES = 1_000
...
function cleanActorDisplayName(value: unknown, fallback: string): string {
...
function cleanActorHandle(value: unknown, fallback: string): string {
  const handle = stringValue(value)
    ...
    .replace(/_+/g, '_')
    .replace(/^_+|_+$/g, '')
    .slice(0, MAX_ACTOR_HANDLE_LENGTH)
  return handle || toHandle(fallback, 'actor')
}

function applyActorIdentity(actor: TimelineActor, identity?: TimelineActorIdentity): TimelineActor {
  return identity
    ? { ...actor, name: identity.displayName, handle: identity.handle }
    : actor
}

function actorIdentityBlock(actor: TimelineActor, label = 'RESPONDING ACTOR'): string {
  return [
...
const ACTOR_IDENTITY_GUARD = 'Display names are not unique. Identify every participant by stable_actor_key, not by name. Speak only as the RESPONDING ACTOR and never merge their identity, profile, memories, or voice with a same-named participant.'

function applyActorIdentitiesToDirectory(directory: TimelineDirectory, state: TimelineState): TimelineDirectory {
...
function applyActorIdentitiesToState(state: TimelineState): TimelineState {
...
function stripLeadingActorAttribution(text: string, actor: TimelineActor): string {
  const identities = [`@${actor.handle}`, actor.name]
    .map((identity) => escapeRegExp(identity.trim()))
    .filter(Boolean)
    .join('|')
  if (!identities) return text

  // Keep ordinary @mentions intact. Only remove an exact self-attribution at
  // the beginning when the model echoes chat-style speaker formatting.
...
function normalizeActorIdentity(value: unknown): TimelineActorIdentity | null {
  if (!isRecord(value)) return null
...
function normalizeState(value: unknown): TimelineState {
  ...
  const actorIdentities = isRecord(value.actorIdentities)
    ? Object.fromEntries(
      Object.entries(value.actorIdentities)
        .slice(0, MAX_SAVED_ACTOR_IDENTITIES)
        .map(([key, identity]) => [key, normalizeActorIdentity(identity)] as const)
        .filter((entry): entry is readonly [string, TimelineActorIdentity] => Boolean(entry[0] && entry[1])),
    )
    : {}
  return applyActorIdentitiesToState({
    ...
    rosterActionHistory: Array.isArray(value.rosterActionHistory)
      ? value.rosterActionHistory
        .filter((action): action is TimelineRosterAction => action === 'weave' || action === 'reply' || action === 'react' || action === 'dm')
...
function formatThread(thread: TimelinePost[], respondingActor: TimelineActor): string {
...
async function extractAndResolveGif(
  content: string,
  contentLimit: number,
  outputActor?: TimelineActor,
): Promise<{ content: string; gifUrl?: string; reaction?: string }> {
  let cleanContent = content
  let reaction: string | undefined

  const gifDirectivePatterns = [
    // Remove an unnecessary code fence along with an otherwise valid final tag.
    /(?:^|\n)[ \t]*```(?:xml|html)?[ \t]*\n[ \t]*<gif\s*>\s*([^<>\r\n]+?)\s*<\/gif\s*>[ \t]*\n[ \t]*```[ \t]*$/i,
    // Accept the required format, including harmless whitespace in the tags.
    /<gif\s*>\s*([^<>\r\n]+?)\s*<\/gif\s*>/i,
    // Recover common alternative tag names and BBCode-style tags.
    /<(?:gif[_-]?query|tenor)\s*>\s*([^<>\r\n]+?)\s*<\/(?:gif[_-]?query|tenor)\s*>/i,
    /\[gif\]\s*([^\[\]\r\n]+?)\s*\[\/gif\]/i,
    // Recover a malformed final `<gif>` line with a missing or partial close.
    /(?:^|\n)[ \t]*<gif\s*>\s*([^<>\r\n]+?)\s*(?:<\/gif\s*)?>?[ \t]*$/i,
    // Recover a final plain-text directive without interpreting ordinary prose.
    /(?:^|\n)[ \t]*(?:gif(?:[ \t]+(?:search|query))?|tenor(?:[ \t]+search)?)\s*:\s*([^\r\n]{1,120}?)[ \t]*$/i,
  ]
  let gifTag: RegExpMatchArray | null = null
  ...
  if (outputActor) cleanContent = stripLeadingActorAttribution(cleanContent, outputActor)
...
function unavailableActorHandles(state: TimelineState, directory: TimelineDirectory, actorKey: string): Set<string> {
  const unavailable = new Set<string>()
  for (const actor of [...directory.personas, ...directory.replyActors]) {
    if (actor.key === actorKey) continue
  ...
  for (const [key, identity] of Object.entries(state.actorIdentities)) {
    if (key !== actorKey) unavailable.add(identity.handle.toLowerCase())
  }
  return unavailable
}

function uniqueActorHandle(preferred: string, unavailable: Set<string>): string {
...
function actorIdentityMessages(actor: TimelineActor, unavailable: Set<string>): LlmMessageDTO[] {
...
function parseActorIdentity(content: string, actor: TimelineActor, unavailable: Set<string>): TimelineActorIdentity {
...
async function ensureActorIdentity(
  ...
  if (existing && !force) return applyActorIdentity(actor, existing)

  const unavailable = unavailableActorHandles(state, directory, actor.key)
  const result = await runSidecar(
    state,
    directory,
    actorIdentityMessages(actor, unavailable),
    userId,
    400,
  )
...
function actorsMissingClaimedIdentity(state: TimelineState, directory: TimelineDirectory): TimelineActor[] {
  const actorsByKey = new Map<string, TimelineActor>()
  for (const post of state.posts) {
    if (post.author.kind !== 'persona') actorsByKey.set(post.author.key, post.author)
  }
  for (const thread of state.directThreads) actorsByKey.set(thread.actor.key, thread.actor)
  for (const actor of directory.replyActors) actorsByKey.set(actor.key, actor)

  const representedActorKeys = new Set<string>(state.rosterActorKeys)
  for (const post of state.posts) {
    if (post.author.kind !== 'persona') representedActorKeys.add(post.author.key)
  }
  for (const thread of state.directThreads) representedActorKeys.add(thread.actor.key)

  return [...representedActorKeys]
    .filter((key) => !state.actorIdentities[key])
    .map((key) => actorsByKey.get(key))
    .filter((actor): actor is TimelineActor => Boolean(actor))
}

function directThreadTranscript(thread: TimelineDirectThread, actor: TimelineActor, persona: TimelineActor): string {
  return thread.messages
    .slice(-18)
    .map((message) => {
      const role = message.direction === 'incoming' && message.author.key === actor.key
        ? 'YOU — RESPONDING ACTOR'
        : message.direction === 'incoming'
          ? 'OTHER ACTOR — PRIOR THREAD CONTEXT ONLY'
          : message.author.key === persona.key
            ? 'DM RECIPIENT — CURRENT USER PERSONA'
            : 'OTHER USER PERSONA — PRIOR THREAD CONTEXT ONLY'
      ...
      content: [
        ...
        mode === 'start'
          ...
          : mode === 'reply'
            ...
            : `Send ${persona.name} a natural follow-up in the existing private thread. If your own message is currently the newest one, do not pretend they replied and do not nag; add a distinct thought that is genuinely worth another message.`,
        ...
        ...(allowGif
          ? ['If a GIF would make the message land better, you may attach one. If you do, the last line must be exactly <gif>SHORT SEARCH QUERY</gif>, with both literal tags and no label, URL, JSON, markdown, or code fence. Keep the query on that one line.']
          : []),
        `PROFILE:\n${actor.profile || actor.bio}`,
      ].join('\n\n'),
    },
...
function recentWeaveContext(posts: TimelinePost[], limit = RECENT_TIMELINE_CONTEXT_POSTS): string {
  return [...posts]
    .sort((left, right) => left.createdAt - right.createdAt)
    .slice(-limit)
    .map((post) => `[stable_actor_key=${JSON.stringify(post.author.key)} · @${post.author.handle} (${post.author.name})]\n${post.content}`)
    .join('\n')
}
...
function timelineEngagementMessages(
  actor: TimelineActor,
  posts: TimelinePost[],
  userPersona: TimelineActor | null,
  directThread: TimelineDirectThread | null,
  settings: TimelineSettings,
): LlmMessageDTO[] {
  const encourageNsfw = settings.encourageNsfw ?? false
  ...
  return [
    ...
    },
    {
      ...
      content: [
        ...
        userPersona
          ? `USER PERSONA AVAILABLE FOR PRIVATE DM — stable_actor_key=${JSON.stringify(userPersona.key)} · @${userPersona.handle} (${userPersona.name})`
          : 'USER PERSONA AVAILABLE FOR PRIVATE DM: none',
        userPersona
          ? `PRIVATE DM STATE WITH USER PERSONA: ${directThread ? `existing thread; newest message is from ${directThread.messages.at(-1)?.direction === 'outgoing' ? 'the user persona' : 'the responding actor'}` : 'no existing thread'}`
          : '',
...
function selectBalancedRosterAction(state: TimelineState, actor: TimelineActor, canDirectMessage: boolean): TimelineEngagementAction {
  ...
  if (canDirectMessage) actions.push('dm')
...
async function createActorDirectTurn(
  state: TimelineState,
  directory: TimelineDirectory,
  actor: TimelineActor,
  persona: TimelineActor,
  ...
  const mode: 'start' | 'reply' | 'followup' = !thread
    ? 'start'
    : newestMessage?.direction === 'outgoing'
      ? 'reply'
      : 'followup'
  const reply = await runSidecar(
    state,
    directory,
    directMessageMessages(actor, persona, thread, state.posts, state.settings, mode),
    userId,
    MAX_DIRECT_MESSAGE_LENGTH,
    actor,
  )
...
async function sendDirectMessage(payload: UnknownRecord, userId: string): Promise<void> {
  ...
  const actor = await ensureActorIdentity(
    state,
    directory,
    getReplyActor(directory, thread.actor.key, thread.actor),
    userId,
  )
  thread.actor = actor
  const persona = getPersonaAuthor(directory, payload.personaId, state.settings)
  ...
  try {
    const reply = await runSidecar(
      state,
      directory,
      directMessageMessages(actor, persona, thread, state.posts, state.settings, 'reply'),
      userId,
      MAX_DIRECT_MESSAGE_LENGTH,
      actor,
    )
...
async function createActorReply(
  ...
  const actor = await ensureActorIdentity(
    state,
    directory,
    getReplyActor(directory, actorKey, fallbackActor),
    userId,
  )
  ...
  try {
    ...
    const { content, gifUrl } = await runSidecar(
      state,
      directory,
      replyMessages(actor, target, thread, state.settings, chatContext),
      userId,
      MAX_WEAVE_LENGTH,
      actor,
    )
...
async function createActorReplies(
  ...
  const preparedActors: TimelineActor[] = []
...
async function createActorWeave(payload: UnknownRecord, userId: string): Promise<void> {
  ...
  try {
    const { content, gifUrl } = await runSidecar(
      state,
      directory,
      originalWeaveMessages(actor, state.settings, state.posts, userPersona),
      userId,
      MAX_WEAVE_LENGTH,
      actor,
    )
...
async function createScheduledRosterWeave(userId: string): Promise<void> {
  ...
  try {
    ...
    const decisionPosts = [...state.posts]
      .filter((post) => post.author.key !== actor.key)
      .sort((left, right) => right.createdAt - left.createdAt)
      .slice(0, RECENT_TIMELINE_CONTEXT_POSTS)
    const existingDirectThread = state.directThreads.find((thread) => thread.actor.key === actor.key) ?? null
    const engagement = await runSidecar(
      state,
      directory,
      timelineEngagementMessages(actor, decisionPosts, dmPersona, existingDirectThread, state.settings),
      userId,
    )
...
async function generateActorIdentity(payload: UnknownRecord, userId: string): Promise<void> {
  const [state, directory] = await Promise.all([loadState(userId), loadDirectory(userId)])
  const actor = getReplyActor(directory, payload.actorKey)
  sendActivity(userId, true, actor.name)
  try {
...
async function backfillActorIdentities(userId: string): Promise<void> {
  const [state, directory] = await Promise.all([loadState(userId), loadDirectory(userId)])
