...
- A global **Timeline** drawer tab and a **Weave current chat** action in the chat input Extras menu. It immediately publishes the latest human message as the selected persona, then automatically invites the active chat character to respond; chat weaves can also save a configurable snapshot of recent messages for that reply.
...
- Chat weaves use the latest human message verbatim after HTML is stripped. When enabled, their configurable plain-text context snapshot is retained privately for thread replies.
